...
8、访问所有可下载的应用程序
    8.1 简要说明
        客户端输入一个关键词，服务器进行模糊匹配，返回最多10个结果
    8.2 事件流
        基本流
            (1) 客户端向服务器发送报文：
            {                    
            ...
            (2) 服务器向用户返回游戏的基本信息，以及下载链接，目前的想法是，基本信息包括：游戏名
            {
                Response:{
                        "games" :[
                            [
...
1、上架应用
    1.1 简要说明
        管理员向平台上架一款应用，要求有应用的名字，有应用的下载链接
    1.2 事件流
        基本流
            (1) 客户端向服务器发送一个请求，内容如下：
                {
                    'name' : '',
                    'download_url' : ''
                }
            (2) 服务器向数据库存储这个信息，sql语句如下：
                insert into game_list values(name,download_url);
            (3) 服务器向用户返回报文，表示存储信息是否成功
                {
                    'success' : ture | false,
                    'mess' : ''
                }
